...
A *canonical* PURL string adheres to [the ECMA-427 2nd Edition ABNF grammar](https://github.com/package-url/purl-spec/blob/main/docs/specification/standard/Annex-C-ABNF-Grammar.md).
...
The following grammar deliberately over-accepts: it describes the superset of
inputs that a PURL parser should attempt to process, rather than a second
conformance class. A string matching this grammar is not thereby a valid
PURL.

Parsers should accept such input on a best-effort basis, normalize it
according to the parsing and canonicalization rules of [the EMCMA-427 standard](docs/specification/standard/Clause-5-Package-URL-Specification.md) and the ["How to parse a PURL" specification](docs/specification/how-to-parse.md)., and reject it only
where no meaningful interpretation is possible.
...
- When a string admits multiple parses under this grammar, component
  boundaries are determined by the parsing rules of [the EMCMA-427 standard](docs/specification/standard/Clause-5-Package-URL-Specification.md) and the ["How to parse a PURL" specification](docs/specification/how-to-parse.md).