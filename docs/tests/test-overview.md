...
- Enable tools to demonstrate conformance with the PURL specification as
  defined in [ECMA-427](https://ecma-tc54.github.io/ECMA-427/) or in
  registered PURL **type** definitions
...
- 'build': A test case for the function of building a canonical PURL output
  string from an input of decoded PURL components. See also
  [`/docs/how-to-build.md`](https://github.com/package-url/purl-spec/blob/main/docs/specification/how-to-build.md).
...
- 'roundtrip': A test case for the function of validating a PURL input
  ...
  case to:
   - Test the "round trip" process of parsing an input PURL string into
     an object of decoded PURL components and then building a PURL string from
     the components, or
...
a syntactically invalid PURL and a PURL input that fails PURL
