...
|------|-------------|-------------|---------------|-------------|
| repository_url | Required | Repository URL |  | The canonical origin of the OTP application source. Since there is no default package repository for `otp`, this qualifier is required and must point to a trusted source repository (e.g. a GitHub project, GitLab repo, or other VCS host). |
| vcs_url | Required | VCS URL |  | Specifies the precise version control system location of the OTP application (e.g. `git+https://github.com/elixir-lang/elixir.git`). |
| download_url | Optional | Download URL |  | Specifies a direct URL to a release archive or source tarball (e.g. `https://github.com/erlang/otp/releases/download/OTP-27.0/otp_src_27.0.tar.gz`). |
